...
int CMsg_bPut(CMsg*This, char cFrom, char * pData, unsigned  int dwSize, int iIndex, char cKey)
{
 ...
 void  DestroyMsgQuene()
{
 ...
 int bPutMsgQuene(char cFrom, char * pData, unsigned  int dwMsgSize, int iIndex, char cKey)
 {
	 ...
	 if (CMsg_bPut(m_pMsgQuene[m_iQueneTail], cFrom, pData, dwMsgSize, iIndex, cKey) == 0) return 0;
 ...
 void MsgProcess()
 {
	 ...
	 for (int i = 0; i < DEF_MSGBUFFERSIZE; i++)
		 m_pMsgBuffer[i] = 0;
