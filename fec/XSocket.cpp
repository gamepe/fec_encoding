...
int XSocket_bInitBufferSize(XSOCKET *This, unsigned long  dwBufferSize)
{
	...
	if (This->m_pRcvBuffer == NULL) return FALSE;
...
int  XSocket_iOnRead(XSOCKET *This,VOID *pData,unsigned short Size)
{
 int iRet;
 unsigned short   * wp;
 ...
 if (This->m_cStatus == DEF_XSOCKSTATUS_READINGHEADER) {
		...
		if (This->m_dwReadSize == 0) { //always TRUE
		...
		if (Size > iRet) {
			Size = Size - iRet;
			pData = (unsigned char*)pData + iRet;
			XSocket_iOnRead(This, pData, Size);
		}
...
void XSocket_OnArenaRead(XSOCKET *This)
{
	...
	if (bPutMsgQuene(1, pData, dwMsgSize, NULL, cKey) == FALSE) {
